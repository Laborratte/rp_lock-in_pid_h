...
int lock_update(rp_app_params_t *params)
{
  ...
  //g_lock_reg->cnt_clk2                  = (int)params[LOCK_CNT_CLK2                 ].value;
    ...
    g_lock_reg->pidA_rst_val              = (int)params[LOCK_PIDA_RST_VAL             ].value;
  ...
  //g_lock_reg->ctrl_A                    = (int)params[LOCK_CTRL_A                   ].value;
    ...
    g_lock_reg->pidB_rst_val              = (int)params[LOCK_PIDB_RST_VAL             ].value;
...
int lock_update_main(rp_app_params_t *params)
{
    ...
    params[170].value = (float)g_lock_reg->pidA_rst_val          ; // lock_pidA_rst_val
    params[171].value = (float)g_lock_reg->pidA_in               ; // lock_pidA_in
    params[172].value = (float)g_lock_reg->pidA_out              ; // lock_pidA_out
    params[173].value = (float) ((g_lock_reg->pidA_ctrl                 )& 0x001) ; // lock_pidA_irst
    params[174].value = (float) ((g_lock_reg->pidA_ctrl            >>1  )& 0x001) ; // lock_pidA_freeze
    params[175].value = (float) ((g_lock_reg->pidA_ctrl            >>2  )& 0x001) ; // lock_pidA_ifreeze
    params[176].value = (float)g_lock_reg->ctrl_A                ; // lock_ctrl_A
    params[177].value = (float)g_lock_reg->pidB_sw               ; // lock_pidB_sw
    params[178].value = (float)g_lock_reg->pidB_PSR              ; // lock_pidB_PSR
    params[179].value = (float)g_lock_reg->pidB_ISR              ; // lock_pidB_ISR
    params[180].value = (float)g_lock_reg->pidB_DSR              ; // lock_pidB_DSR
    params[181].value = (float)g_lock_reg->pidB_SAT              ; // lock_pidB_SAT
    params[182].value = (float)g_lock_reg->pidB_sp               ; // lock_pidB_sp
    params[183].value = (float)g_lock_reg->pidB_kp               ; // lock_pidB_kp
    params[184].value = (float)g_lock_reg->pidB_ki               ; // lock_pidB_ki
    params[185].value = (float)g_lock_reg->pidB_kd               ; // lock_pidB_kd
    params[186].value = (float)g_lock_reg->pidB_rst_val          ; // lock_pidB_rst_val
    params[187].value = (float)g_lock_reg->pidB_in               ; // lock_pidB_in
    params[188].value = (float)g_lock_reg->pidB_out              ; // lock_pidB_out
    params[189].value = (float) ((g_lock_reg->pidB_ctrl                 )& 0x001) ; // lock_pidB_irst
    params[190].value = (float) ((g_lock_reg->pidB_ctrl            >>1  )& 0x001) ; // lock_pidB_freeze
    params[191].value = (float) ((g_lock_reg->pidB_ctrl            >>2  )& 0x001) ; // lock_pidB_ifreeze
    params[192].value = (float)g_lock_reg->ctrl_B                ; // lock_ctrl_B
    params[193].value = (float)g_lock_reg->aux_A                 ; // lock_aux_A
    params[194].value = (float)g_lock_reg->aux_B                 ; // lock_aux_B
    params[195].value = (float) (( g_lock_reg->lock_feedback >> 0 ) & 0x01 ) ; // lock_ctrl_aux_lock_now
    params[196].value = (float) (( g_lock_reg->lock_feedback >> 1 ) & 0x01 ) ; // lock_ctrl_aux_launch_lock_trig
    params[197].value = (float) (( g_lock_reg->lock_feedback >> 2 ) & 0x01 ) ; // lock_ctrl_aux_pidB_enable_ctrl
    params[198].value = (float) (( g_lock_reg->lock_feedback >> 3 ) & 0x01 ) ; // lock_ctrl_aux_pidA_enable_ctrl
    params[199].value = (float) (( g_lock_reg->lock_feedback >> 4 ) & 0x01 ) ; // lock_ctrl_aux_ramp_enable_ctrl
    params[200].value = (float) (( g_lock_reg->lock_feedback >> 5 ) & 0x01 ) ; // lock_ctrl_aux_set_pidB_enable
    params[201].value = (float) (( g_lock_reg->lock_feedback >> 6 ) & 0x01 ) ; // lock_ctrl_aux_set_pidA_enable
    params[202].value = (float) (( g_lock_reg->lock_feedback >> 7 ) & 0x01 ) ; // lock_ctrl_aux_set_ramp_enable
    params[203].value = (float) (( g_lock_reg->lock_feedback >> 8 ) & 0x03 ) ; // lock_ctrl_aux_trig_type
    params[204].value = (float) (( g_lock_reg->lock_feedback >>10 ) & 0x01 ) ; // lock_ctrl_aux_lock_trig_rise
